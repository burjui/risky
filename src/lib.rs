...
pub mod instructions {
    ...
    pub fn ecall() -> Result<u32> {
        i_instruction("ecall", opcode::SYSTEM, 0b00000, 0b000, 0b00000, 0b0000_0000_0000)
    }

    pub fn ebreak() -> Result<u32> {
        i_instruction("ebreak", opcode::SYSTEM, 0b00000, 0b000, 0b00000, 0b0000_0000_0001)
    }
...
fn fence_instruction(function_name: &'static str, funct3: u8, pred: u8, succ: u8) -> Result<u32> {
    check_funct3(function_name, funct3)?;
    check_range(function_name, "pred", pred, 0 .. 1 << 4)?;
    check_range(function_name, "succ", succ, 0 .. 1 << 4)?;
...
fn fence_flag_error(kind: &'static str, flag_name: char, mask_str: &str) -> String {
    format!("{} fence flag name `{}' in fence mask `{}'", kind, flag_name, mask_str)
}

fn csr_instruction(function_name: &'static str, rd: u8, funct3: u8, rs1: u8, csr: u16) -> Result<u32> {
    i_instruction(function_name, opcode::SYSTEM, rd, funct3, rs1, csr as i16)
}

fn r_instruction(function_name: &'static str, opcode: u8, rd: u8, funct3: u8, rs1: u8, rs2: u8, funct7: u8) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_register(function_name, rd)?;
    check_funct3(function_name, funct3)?;
    check_register(function_name, rs1)?;
    check_register(function_name, rs2)?;
    check_funct7(function_name, funct7)?;
    Ok(u32::from(opcode) |
        (u32::from(rd) << 7) |
        (u32::from(funct3) << 12) |
        (u32::from(rs1) << 15) |
...
fn i_instruction(function_name: &'static str, opcode: u8, rd: u8, funct3: u8, rs1: u8, imm: i16) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_register(function_name, rd)?;
    check_funct3(function_name, funct3)?;
    check_imm_i_s(function_name, imm)?;
    Ok(u32::from(opcode) |
        (u32::from(rd) << 7) |
        (u32::from(funct3) << 12) |
        (u32::from(rs1) << 15) |
...
fn s_instruction(function_name: &'static str, opcode: u8, imm: i16, funct3: u8, rs1: u8, rs2: u8) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_imm_i_s(function_name, imm)?;
    check_funct3(function_name, funct3)?;
    check_register(function_name, rs1)?;
...
fn b_instruction(function_name: &'static str, opcode: u8, imm: i16, funct3: u8, rs1: u8, rs2: u8) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_imm_b(function_name, imm)?;
    check_funct3(function_name, funct3)?;
    check_register(function_name, rs1)?;
    check_register(function_name, rs2)?;
    let imm_u32 = imm as u32;
    Ok(u32::from(opcode) |
        (u32::from((imm_u32 >> 11) & 0b1) << 7) |
        (u32::from((imm_u32 >> 1) & 0b1111) << 8) |
        (u32::from(funct3) << 12) |
        (u32::from(rs1) << 15) |
        (u32::from(rs2) << 20) |
        (u32::from((imm_u32 >> 5) & 0b11_1111) << 25) |
        (u32::from((imm_u32 >> 12) & 0b1) << 31))
}

fn u_instruction(function_name: &'static str, opcode: u8, rd: u8, imm: i32) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_register(function_name, rd)?;
    Ok(u32::from(opcode) |
        (u32::from(rd) << 7) |
...
fn j_instruction(function_name: &'static str, opcode: u8, rd: u8, imm: i32) -> Result<u32> {
    check_opcode(function_name, opcode)?;
    check_register(function_name, rd)?;
    check_imm_j(function_name, imm)?;
    let imm_u32 = imm as u32;
    let imm_field =
        (imm_u32 >> 12 & 0b1111_1111) << 12 |
        (imm_u32 >> 11 & 0b1) << 20 |
        (imm_u32 >> 1 & 0b11_1111_1111) << 21 |
        (imm_u32 >> 20 & 0b1) << 31;
    Ok(u32::from(opcode) | (u32::from(rd) << 7) | imm_field)
}
...
fn check_range<T>(function_name: &'static str, value_name: &'static str, value: T, range: Range<T>) -> Result<()>
    where T: PartialOrd + Display + fmt::LowerHex
{
